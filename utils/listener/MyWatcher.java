...
/**
 *
 * 规定TextView或Edittext 小数点前只能填几位，小数点后填几位
 *
 * input.addTextChangedListener(new MyWatcher(6,2) {
 });
...
public abstract class MyWatcher implements TextWatcher {
    ...
    private void judge(Editable editable) {
        ...
        if (posDot > 0 && posDot > beforeDot && beforeDot != -1) {
            if (temp.length() <= beforeDot) {
                //do nothing 仅仅为了理解逻辑而已
            } else {
                editable.delete(beforeDot - 1, beforeDot);
            }
        }
